...
namespace StarwarsOpg
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            #region Opgave 1
            // Opgave 1
            var opg1List = from p in planets where p.Name.StartsWith("M") == true select p;
            Console.WriteLine("Opg2");
            foreach (var planet in opg1List)
            {
            ...
            #region Opgave 2
            //Opgave 2
            var opg2List = from p in planets where p.Name.Contains("y") == true || p.Name.Contains("Y") select p;
            Console.WriteLine("\nOpg2");
            foreach (var planet in opg2List)
            {
                Console.WriteLine(planet.Name);
            }
            #endregion
            #region Opgave 3
            //Opgave 3
            var opg3List = from p in planets where p.Name.Length > 9 && p.Name.Length < 15 select p;
            Console.WriteLine("\nOpg3");
            foreach (var planet in opg3List)
            {
                Console.WriteLine(planet.Name);
            }
            #endregion
            #region Opgave 4
            //Opgave 4
            var opg4List = from p in planets
                           where p.Name[1].ToString().Equals("a") && p.Name.EndsWith("e")
                           select p;
            Console.WriteLine("\nOpg4");
            foreach (var planet in opg4List)
            {
                Console.WriteLine(planet.Name);
            }
            #endregion
            #region Opgave 5
            //Opgave 5
            var opg5List = from p in planets
                           where p.RotationPeriod > 40
                           orderby p.RotationPeriod
                           select p;
            Console.WriteLine("\nOpg5");
            foreach (var planet in opg5List)
            {
                Console.WriteLine(planet.Name);
            }
            #endregion
            ...
            var opg6List = from p in planets
                           where p.RotationPeriod > 10 && p.RotationPeriod < 20
                           orderby p.Name
                           select p;
            Console.WriteLine("\nOpg 6");
            foreach (var planet in opg6List)
            {
                Console.WriteLine(planet.Name);
            }
            #endregion
            ...
            var opg7List = planets.Where(p => p.RotationPeriod > 30).OrderBy(o => o.Name).ThenBy(o => o.RotationPeriod);
            Console.WriteLine("\nOpg 7");
            foreach (var planet in opg7List)
            {
                Console.WriteLine(planet.Name);
            }
            #endregion
            ...
            var opg8List = planets.Where(r => r.RotationPeriod < 30 || r.SurfaceWater > 50).Where(p => p.Name.Contains("ba")).OrderBy(p => p.Name).ThenBy(p => p.SurfaceWater).ThenBy(p => p.RotationPeriod);
            Console.WriteLine("\nOpg 8");
            foreach (var planet in opg8List)
            {
                Console.WriteLine(planet.Name);
            }
            #endregion
            ...
            var opg9List = planets.Where(p => p.SurfaceWater > 0).OrderByDescending(p => p.SurfaceWater);
            Console.WriteLine("\nOpg 9");
            foreach (var planet in opg9List)
            {
                Console.WriteLine(planet.Name);
            }
            #endregion
            ...
            Console.WriteLine("\nOpg 10");
            var opg10List = planets.FindAll(p => p.Diameter != 0 && p.Population != 0)
                .OrderBy(p => CalcArea(p.Diameter, p.Population));
            foreach (var planet in opg10List)
            {
                Console.WriteLine(planet.Name);
            }
            #endregion
            ...
            Console.WriteLine("\nOpg 11");
            var opg11List = planets.Except(planets.Where(p => p.RotationPeriod > 0));
            foreach (var planet in opg11List)
            {
                Console.WriteLine(planet.Name);
            }
            #endregion
            ...
            Console.WriteLine("\nOpg 12");
            var opg12List = planets.Where(p => p.Name.StartsWith("A") || p.Name.EndsWith("s"));
            var opg12List2 = planets.Where(p => p.Terrain != null && p.Terrain.Count > 0).Where(x => x.Terrain.FindAll(u => u == "rainforests").Any());
            var opg12FinalList = opg12List.Union(opg12List2);
            foreach (var planet in opg12FinalList)
            {
                Console.WriteLine(planet.Name);
            }
            #endregion
            ...
            Console.WriteLine("\nOpg 13");
            var opg13List = planets.Where(p => p.Terrain != null && p.Terrain.Count > 0)
                .Where(u => u.Terrain.FindAll(x => x == "deserts" || x == "desert" || x == "rocky deserts").Any());
            foreach (var planet in opg13List)
            {
                Console.WriteLine(planet.Name);
            }
            #endregion
            ...
            Console.WriteLine("\nOpg 14");
            var opg14List = planets.Where(p => p.Terrain != null)
                .Where(x => x.Terrain.FindAll(u => u.Equals("swamp") || u.Equals("swamps")).Any()).OrderBy(p => p.RotationPeriod)
                .ThenBy(p => p.Name);
            foreach (var planet in opg14List)
            {
                Console.WriteLine(planet.Name);
            }
            #endregion
            ...
            Console.WriteLine("\nOpg 15");
            Regex reg = new Regex(@"([aeiouy])\1");
            var opg15List = planets.Where(p => reg.Matches(p.Name).Count > 0);
            foreach (var planet in opg15List)
            {
                Console.WriteLine(planet.Name);
            }
            #endregion
            ...
            var opg16List = planets.Where(p => reg2.IsMatch(p.Name)).OrderByDescending(x => x.Name);
            foreach (var planet in opg16List)
            {
                Console.WriteLine(planet.Name);
            }
            #endregion
        ...
        public static double CalcArea(int diameter, long population)
        {
